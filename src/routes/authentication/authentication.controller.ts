...
class AuthenticationController {
  ...
  public loginUser = async (root, args) => {
    const errors = await schemaValidation(loginSchema, args);

    if (errors) {
      return validationError('Bad request', 400, errors);
    }

    const logInData: LogInDto = args;

    try {
      const authToken = await this.authenticationService.login(logInData);
